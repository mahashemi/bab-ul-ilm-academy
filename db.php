...
- content: REAL educational content the student will actually read and learn from — never a placeholder, summary, outline, or text like "[lesson content here]". Write 150-400 words of substantive, accurate teaching material for EVERY single row: explain concepts clearly, define key terms, and include a short example where useful. If a reference textbook was given above, align the content and terminology with it. Do not shorten or skip this for later rows even if the course has many lessons — every row gets the full treatment.
  This field is RICH TEXT: format the content with simple HTML for readability — <p> paragraphs, <h2>/<h3> subheadings, <strong>/<em> emphasis, <ul>/<ol> with <li> bullet points, <blockquote> for quotes (e.g. Quranic verses or hadith), <code>/<pre> for code or formulas, and <hr> as a divider. Supported tags: p, br, strong, b, em, i, u, s, h1, h2, h3, ul, ol, li, a (href only), blockquote, code, pre, hr. Anything else (scripts, iframes, styles, images) is stripped on upload, so don't use it. IMPORTANT for CSV validity: keep each lesson's entire HTML on its one CSV row — use <p>/<br> for line breaks, never a raw newline inside the cell — and prefer single quotes in HTML attributes (e.g. <a href='...'>) so you don't have to escape double quotes in the CSV.
