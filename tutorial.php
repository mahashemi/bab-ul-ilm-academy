...
$STEPS = [
    'en' => [
        ...
        'steps' => [
            ...
            ['icon' => 'upload', 'title' => 'Have a lot of content? Upload it from a CSV instead of typing it by hand', 'body' => 'Every section above also has an upload option that accepts a CSV file — useful if you already have your course mapped out, or want an AI to help write it. Each upload page shows the exact columns required, lets you download a starter template, and gives you a ready-made prompt you can hand to an AI assistant to generate the file for you. If anything in your file fails validation, the page shows exactly which rows/fields failed and gives you a second ready-made prompt — with your file and the errors already included — to paste into an AI and get a corrected version back. Start with <a href="single-upload.php">Single Upload</a> to create a course from a CSV, then use its "Bulk Upload" buttons (or the ones on Edit Course) for lessons, quizzes, and assignments.'],
    ...
    'fa' => [
        ...
        'steps' => [
            ...
            ['icon' => 'upload', 'title' => 'محتوای زیادی دارید؟ آن را از یک فایل CSV آپلود کنید، نه با تایپ دستی', 'body' => 'هر بخش بالا دارای گزینه آپلود است که یک فایل CSV می‌پذیرد — مفید برای زمانی که محتوای دوره را از قبل آماده دارید یا می‌خواهید یک هوش مصنوعی در نوشتن آن کمک کند. هر صفحه آپلود، ستون‌های دقیق لازم را نشان می‌دهد، امکان دانلود یک الگوی نمونه را فراهم می‌کند و یک متن آماده در اختیار شما قرار می‌دهد که می‌توانید به یک دستیار هوش مصنوعی بدهید تا فایل را برایتان بسازد. اگر فایل شما خطا داشته باشد، صفحه دقیقاً نشان می‌دهد کدام ردیف‌ها/فیلدها مشکل دارند و یک متن آماده دوم -- همراه با فایل و خطاهای شما -- در اختیارتان می‌گذارد تا به هوش مصنوعی بدهید و نسخه اصلاح‌شده را پس بگیرید. با <a href="single-upload.php">آپلود تکی</a> برای ساخت یک دوره از فایل CSV شروع کنید، سپس از دکمه‌های «آپلود گروهی» (در همان صفحه یا صفحه ویرایش دوره) برای درس‌ها، آزمون‌ها و تمرین‌ها استفاده کنید.'],
    ...
    'ur' => [
        ...
        'steps' => [
            ...
            ['icon' => 'upload', 'title' => 'بہت زیادہ مواد ہے؟ ہاتھ سے ٹائپ کرنے کے بجائے CSV فائل سے اپلوڈ کریں', 'body' => 'اوپر دیئے گئے ہر حصے میں اپلوڈ کا آپشن بھی موجود ہے جو CSV فائل قبول کرتا ہے — یہ اس وقت مفید ہے جب آپ کے پاس پہلے سے کورس کا مواد تیار ہو، یا آپ چاہتے ہوں کہ ایک AI اسے لکھنے میں مدد کرے۔ ہر اپلوڈ صفحہ درکار درست کالمز دکھاتا ہے، ایک نمونہ ٹیمپلیٹ ڈاؤن لوڈ کرنے کی سہولت دیتا ہے، اور ایک تیار شدہ پرامپٹ فراہم کرتا ہے جو آپ کسی AI اسسٹنٹ کو دے کر فائل بنوا سکتے ہیں۔ اگر آپ کی فائل میں کوئی خامی ہو تو صفحہ بالکل واضح طور پر بتاتا ہے کہ کون سی قطاریں/فیلڈز غلط ہیں اور ایک دوسرا تیار شدہ پرامپٹ -- جس میں آپ کی فائل اور خامیاں شامل ہوں -- فراہم کرتا ہے تاکہ آپ AI سے درست شدہ نسخہ حاصل کر سکیں۔ ایک کورس CSV سے بنانے کے لیے <a href="single-upload.php">سنگل اپلوڈ</a> سے شروع کریں، پھر اسباق، کوئزز اور اسائنمنٹس کے لیے "بلک اپلوڈ" بٹن (اسی صفحے یا ایڈٹ کورس پر) استعمال کریں۔'],
